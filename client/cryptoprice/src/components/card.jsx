...
const Card = ({ item }) => {
  const difference = (
    ((item.last - item.open24h) / item.open24h) *
    100
  ).toFixed(2);
  ...
  useEffect(() => {
    if (difference < 0) {
      setDiffClass(
        "float-right rounded-full bg-rose-100 px-1 text-sm font-medium text-rose-600"
      );
    } else {
      setDiffClass(
        "float-right rounded-full bg-green-100 px-1 text-sm font-medium text-green-600"
      );
    }
  }, []);
  return (
